...
pub fn draw(frame: &mut Frame, app: &App, area: Rect) {
    ...
    let input_text = if app.search_input.is_empty() && !is_typing {
        Span::styled("Press / to search tracks, artists, albums...", Theme::dim())
    } else {
        Span::styled(&app.search_input, Theme::text())
    };
    ...
    if is_typing {
        let cursor_x = chunks[0].x + 1 + app.search_input.len() as u16;
    ...
    if app.search_loading {
        ...
        let mut list_state = ListState::default().with_selected(Some(app.search_selected));
